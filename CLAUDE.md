...
Targets: `compile`, `test`, `nuget-pack`, `sonar`, `update-build-version`. `Configuration` defaults to `Debug` locally, `Release` on CI. `nuget-pack` outputs to `artifacts/` and runs with `--no-build --no-restore`, so it needs `compile` to have run in the same configuration.
...
CI is AppVeyor (`appveyor.yml`): `build.cmd update-build-version sonar test nuget-pack --configuration Release`.

**The root-level `Build.cs` is dead legacy code** — it is not part of any project, does not compile, and is not what runs (it still points at the old `src/*.sln` path). Edit `build/Build.cs` instead.
...
- Bumping the released version means editing `<VersionPrefix>` in `src/DotNet.Consolidate/DotNet.Consolidate.csproj` — AppVeyor reads it via `Solution.DotNet_Consolidate.GetProperty("Version")`.
